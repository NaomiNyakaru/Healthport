...
class PatientProfile(models.Model):
    ...
    # ── AI summary cache ──────────────────────────────────────────────────────
    # Generated by Gemini when a doctor opens a patient's detail page.
    # Cached here so repeated page loads don't call the API on every visit.
    # Invalidated when new records or medications are added (via signals).
    ai_summary           = models.TextField(blank=True)
    ai_summary_generated = models.DateTimeField(null=True, blank=True)
    # Fingerprint of the data that was summarized — used to detect when
    # records/meds have changed since the last summary, so stale summaries
    # are regenerated automatically.
    ai_summary_hash      = models.CharField(max_length=64, blank=True)
